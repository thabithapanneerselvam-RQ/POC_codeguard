...
//   // ── Bearer — reads ALL severity levels, no duplicates ──
//   try {
...
// // Sends up to BATCH_SIZE findings in one API call
// function callGeminiBatch(batch) {
//   return new Promise((resolve) => {

//     // Build one prompt with all findings in the batch
...
//               console.log('  ⚠️  Gemini quota hit — will retry after longer delay')
//             } else {
...
//     if (!geminiResults) {
//       console.log('  ❌ Batch failed — all findings in this batch skipped\n')
//       batch.forEach((finding, idx) => {
//         findingCounter++
//         const conf = getConfidenceScore(finding)
//         console.log(`Finding ${findingCounter}/${findings.length}:`)
...
//         console.log(`  Result: SKIPPED — Gemini quota/error\n`)
//       })
//     } else {
//       // Process each finding in the batch with its Gemini result
//       batch.forEach((finding, idx) => {
//         findingCounter++
//         const conf = getConfidenceScore(finding)
//         const gemini = geminiResults.find(r => r.finding_index === idx + 1)
